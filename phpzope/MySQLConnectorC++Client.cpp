...
int main(int argc, const char *argv[]) {
	...
	try {
		...
		cout << "Executing the Query: \"SELECT * FROM object_ids\" .." << endl;

		/* run a query which returns exactly one result set */
		res = stmt -> executeQuery ("SELECT * FROM class_ids");

		cout << "Retrieving the result set .." << endl;

		/* retrieve the data from the result set and display on stdout */
		retrieve_data_and_print (res, NUMOFFSET, 3, string("class_id"));
		...
		/* retrieve the data from the result set and display on stdout */
		retrieve_data_and_print (res, COLNAME, 3, string ("class_id"));
